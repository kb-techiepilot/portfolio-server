...
router.post('/:id', async (req, res) => {
    ...
    if(holdingsData !== undefined) {
        if(holdingsData.QUANTITY < Number(quantity) ) {
            return res.status(404).json({"message" : "Insufficient quantity in funds"});
        } else if(partial) {
            const newQuantity = Number(holdingsData.QUANTITY) - Number(quantity);
            if(newQuantity > 0) {
        ...
        try{
            ...
            res.status(404).json({"message" : "Error while selling share " + err})
        }
    } else {
        res.status(404).json({"message" : "Share not found to sell"});
    }
